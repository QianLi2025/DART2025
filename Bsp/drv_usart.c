...
volatile uint8_t judge_dma_buffer[2][USART5_RX_BUF_LEN] ={0}  ;
uint8_t judge_receive_length=0;
...
//							    USART6_RX_BUF_LEN);  
//}
void USART5_Init(void)     //开启空闲中断，配置DMA相关参数，使能DMA接收
{
	__HAL_UART_CLEAR_IDLEFLAG(&huart5);    //先清除空闲中断标志位，防止开启中断时立马进入中断
...
//	}
//}
void DRV_USART5_IRQHandler(UART_HandleTypeDef *huart)  //在stm32f4xx_it.c文件USART5_IRQHandler调用   
{
	if( __HAL_UART_GET_FLAG(huart, UART_FLAG_IDLE) &&
		__HAL_UART_GET_IT_SOURCE(huart, UART_IT_IDLE))  //判断是否为空闲中断
	{
...
static void uart_rx_idle_callback(UART_HandleTypeDef* huart) 
{
    ...
    if(huart == &huart5)
	{
		...
		judge_receive_length = USART5_RX_BUF_LEN - huart->hdmarx->Instance->NDTR;
		...
		__HAL_DMA_SET_COUNTER(huart->hdmarx, USART5_RX_BUF_LEN);
